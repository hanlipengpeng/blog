...
baidu.dom._styleFixer.textOverflow = (function() {
	...
	return {
		get : function(F, G) {
			var E = baidu.browser;
			return (E.opera ? G.OTextOverflow : E.firefox ? F._baiduOverflow
					: G.textOverflow)
					|| "clip"
		},
		set : function(F, H) {
			...
			E.opera ? (I.OTextOverflow = H)
					: E.firefox ? (F._baiduOverflow = H) : (I.textOverflow = H)
		}
	}
})();
...
(function() {
	...
	var E = B.selectors = {
			...
			"+" : function(W, R) {
				...
				for ( var S = 0, Q = W.length, U; S < Q; S++) {
					if ((U = W[S])) {
						...
						W[S] = X || U && U.nodeName.toLowerCase() === R ? U || false
								: U === R
					}
				}
				if (X) {
		...
		filter : {
			...
			ATTR : function(V, T) {
				var S = T[1], Q = E.attrHandle[S] ? E.attrHandle[S](V)
